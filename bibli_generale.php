...
//minutes, heures , jour, mois, année
function getMinuteHeureJourMoisAnneeFromDate(string $date) : array {
    $t = [];
    $t[] = substr($date, 10, 2);  // minutes as string
    $t[] = substr($date, 8, 2);   // hours as string
    $t[] = intval(substr($date, 6, 2));   // day
    $t[] = getMonthName(intval(substr($date, 4, 2))); // month
    $t[] = intval(substr($date, 0, 4));   // year
    return $t;
}
